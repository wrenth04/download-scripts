// source from kej's FLV Retriever
...
YouTubeParser = {
  ...
  buildVideoUrlHTMLTag: function(item, title, method) {
    return unescape(item.fmt_url) + (item.fmt_sig === false ? "" : "&signature=" + item.fmt_sig) + ' ' + decodeURIComponent(escape(title.replace('"', ''))).replace(/ +/g, '.') + ' ' + method + ' ' + this.fmt_str[item.fmt] + '\n';
  },
...
getYouTubeUrl = function() {
  YouTubeParser.setVideoInfo(str);
